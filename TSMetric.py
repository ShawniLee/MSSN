...
class TSMetric:
    ...
    def _prepare_data(self, values_real, values_pred):
        ...
        assert np.allclose(np.unique(values_real), np.array([0, 1])), f'{values_real}'
        assert np.allclose(np.unique(values_pred), np.array([0, 1])), f'{np.sum(values_pred)}'
        ...
        if self.metric_option == "classic":
            ...
            predicted_anomalies = self.prepare_timeseries(values_pred)
            real_anomalies = self.prepare_timeseries(values_real)
        elif self.metric_option == "numenta":
            predicted_anomalies = np.argwhere(values_pred == 1).repeat(2, axis=1)
            real_anomalies = self.prepare_timeseries(values_real)
        return real_anomalies, predicted_anomalies

    def prepare_timeseries(self, arg0):
        predicted_anomalies_ = np.argwhere(arg0 == 1).ravel()
    ...
    def score(self, values_real, values_predicted):
        ...
        if np.sum(values_predicted) == 0:
            raise NotImplementedError("No anomaly detected!")
